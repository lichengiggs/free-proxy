...
| longcat | `https://api.longcat.chat/openai` | `LONGCAT_API_KEY` | openai | ✓ |
...
**当前策略更新**：
- 已移除 `opencode` 与 `cerebras`
- Gemini 模型列表会过滤 image / vision / embedding 类型
- `free-proxy/coding` 的默认优先级调整为 `longcat -> gemini -> github -> mistral -> sambanova`
- 新增 `nvidia` 作为 OpenAI 兼容高质量后备 provider
...
- Python UI 的模型浏览不能依赖某个 provider 一定实现 `GET /models`。因此 Longcat 和 GitHub/Groq/NVIDIA 一样，在模型列表失败时回退到内置 `model_hints`。
- 首批内置候选模型为：
  - `LongCat-Flash-Chat`
